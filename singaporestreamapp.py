...
st.set_page_config(page_title='HDB Flat Resale Price Predictor',page_icon='house',initial_sidebar_state='expanded',
                        layout='wide',menu_items={"about":'This streamlit application was developed by MADAN.S.S'})
...
with _:
    st.empty()
    st.image('https://raw.githubusercontent.com/MADANSS1/singapore_flat_resale_price_prediction/main/flat.gif')
    st.empty()
...
with st.sidebar:
    selected = option_menu(
        ...
        styles={
            "container": {"padding": "5px","border": "2px ridge ", "background-color": "D997C0"},
            "icon": {"color": 'yellow', "font-size": "25px"},
            "nav-link": {"font-size": "18px", "text-align": "center", "margin": "0px", "--hover-color": "#586e75"},
            "nav-link-selected": {"background-color":"#586e75"},  
                })
...
if selected == "Home":
    ...
    st.markdown('''<h6 style='color:#080708;font-size:20px'> The Housing & Development Board (HDB; often referred to as the Housing Board),
    ...
    with st.container():
        ...
        with cl:
                ...
                st.markdown('''<h6 style='color:#080708;font-size:18px'>An outstanding organisation creating endearing homes all are proud of
                            ''',unsafe_allow_html=True)
        with c2:
                st.subheader(':orange[***Mission***]')
                st.markdown('''<h6 style='color:#080708;font-size:18px'>We provide affordable, quality housing and a great living environment
                            where communities thrive''',unsafe_allow_html=True)
        with c3:
                st.subheader(':orange[***Values***]')
                st.markdown('''<h6 style='color:#080708;font-size:18px'>Our Shared Values form the foundation in all that we do. In all 
    ...
    st.markdown('''<h6 style='color:#080708;font-size:18px'>By the 1940s and 1950s, Singapore experienced rapid population growth, with the 
...
if selected == "About":
    ...
    st.markdown('''**Hello! I'm MADAN.S.S, an MBA graduate. I have an interest in data science and analytics.
                Now i explore the world of data science...**''')
    st.link_button('Linkedin','https://www.linkedin.com/in/madan-subasakthivel-42178a209/')  
